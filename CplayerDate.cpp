...
CplayerDate::CplayerDate() :m_hp(1000),m_maxHp(m_hp), m_mp(500), m_maxMp(m_mp), m_exp(0), m_atk(10), m_def(10), m_critical(5), m_criticalAtk(1.5), m_speed(3.0), m_lv(1), m_gold(1000),m_x(50),m_y(WINSIZEY / 2 + 230)
{
}
...
void CplayerDate::setData(int atk, int def, int hp, int maxhp, int mp, int maxmp, int cri, int lv, int exp, int gold, float criatk, float speed, float x, float y)
{
	m_atk = atk;
	m_def = def;
	m_hp = hp;
